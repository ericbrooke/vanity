module Vanity
  ...
  class Configuration
    ...
    private
      ...
      def self.default_on_datastore_error(error, klass, method, arguments) # :nodoc:
      ...
      def self.default_request_filter(request) # :nodoc:
        request &&
          request.env &&
    ...
    def connection_params(file_name=nil)
      ...
      if File.exists?(file_path)
        ...
        unless params_for_environment
          fail(I18n.t('vanity.failure.no_configuration', environment: environment))
        end
    ...
    def redis_url_from_file
      ...
      if connection_url
        logger.warn(I18n.t('vanity.warning.redis_yml'))
